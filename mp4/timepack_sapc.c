 ...
 *  *accuracy* because of various overhead times involved)i
*/
...
int starttimer()
{
  ...
  int dc = COUNTS_PER_TICK - count;
  startcount = tickcount * COUNTS_PER_TICK  + dc; 
  timer_running = TRUE;
  return T_OK;
}
...
int stoptimer( int *interval )
{
  ...
  count |= (inpt(TIMER0_COUNT_PORT)<<8);	
  int dc = COUNTS_PER_TICK - count;
  int stopcount = tickcount * COUNTS_PER_TICK + dc; 
  *interval = (stopcount - startcount) * USECS_PER_COUNT; 
#ifdef DEBUG
